...
public class Migration
{
    ...
    {
        var filePaths = Directory.GetFiles(directory, ".bit_check", SearchOption.AllDirectories);

        foreach (var filePath in filePaths)
        {
            var jsonString = File.ReadAllText(filePath);
            var fileInfos = new List<HashInfo>();
            var jobject = (JObject)JsonConvert.DeserializeObject(jsonString); // parse as array  
            ...
            foreach (JProperty token in childrenTokens)
            {
                ...
                hashInfo.FileModifyDateTimeUtc =
                    DateTimeOffset.FromUnixTimeSeconds(Convert.ToInt64(token.Value[0])).DateTime;
    ...
    {
        var filePaths = Directory.GetFiles(directory, "hash.json", SearchOption.AllDirectories);
        foreach (var filePath in filePaths)
        {
            ...
            foreach (var hashInfo in hashInfos) hashInfo.Sha1HashCalcDateTimeUtc = lastScannedDateTimeUtc;
